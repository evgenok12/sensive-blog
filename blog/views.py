...
def index(request):
    ...
    posts_with_likes = Post.objects.filter(id__in=most_popular_posts_ids).annotate(Count('comments'))
    ids_and_likes = posts_with_likes.values_list('id', 'comments__count')
    count_for_id = dict(ids_and_likes)
